		...
		if (IF & gb->memory[0xFFFF] & mask) {
			...
			switch (i) 
			{
				case INTBIT_VBLANK: 
					...
					break;
				case INTBIT_STAT:
					gb->memory[0xFF0F] &= ~INTMASK_STAT;
					gb->cpu.PC = 0x48;
					break;
				case INTBIT_TIMER:
					gb->memory[0xFF0F] &= ~INTMASK_TIMER;
					gb->cpu.PC = 0x50;
					break;
				case INTBIT_SERIAL:
					gb->memory[0xFF0F] &= ~INTMASK_SERIAL;
					gb->cpu.PC = 0x58;
					break;
				case INTBIT_JOYPAD:
					gb->memory[0xFF0F] &= ~INTMASK_JOYPAD;
					gb->cpu.PC = 0x60;
					break;
			}
			
			return 20; // Uncertain about this timing
		}
